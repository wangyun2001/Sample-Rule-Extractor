...
def resolve_fields(config: Dict[str, Any]) -> List[str]:
    analysis_fields = config.get("analysis", {}).get("fields", [])
    if isinstance(analysis_fields, list):
        normalized = [str(item).strip() for item in analysis_fields if str(item).strip()]
        if normalized:
            return normalized

    raw_fields = config.get("fields", [])
    fields = [field["field"] if isinstance(field, dict) else str(field) for field in raw_fields]
    fields = [item.strip() for item in fields if item and str(item).strip()]
    if fields:
        return fields
...
def resolve_alias_map(config: Dict[str, Any], fields: List[str]) -> Dict[str, List[str]]:
    alias_map = config.get("analysis", {}).get("field_alias_map", {})
    if isinstance(alias_map, dict) and alias_map:
        return normalize_alias_map(alias_map, fields)

    template_alias = config.get("header_alias", {})
    if isinstance(template_alias, dict) and template_alias:
        normalized_template = {
            str(k): [str(v).strip() for v in values if str(v).strip()]
            for k, values in template_alias.items()
            if isinstance(values, list)
        }
        return normalize_alias_map(normalized_template, fields)

    return normalize_alias_map({}, fields)
...
def symptom_header_gate(headers: List[str], field_map: Dict[int, str]) -> bool:
    ...
    has_mapped_core = ("symptom" in mapped_fields) and (
        ("possible_cause" in mapped_fields) or ("measure" in mapped_fields) or ("repair_action" in mapped_fields)
    )
    return (has_symptom and has_cause_or_measure) or weak_two_col or has_mapped_core


def is_probably_noise_record_for_symptom(record: Dict[str, str], fields: List[str]) -> bool:
    has_core_fields = any(field in fields for field in {"symptom", "possible_cause", "measure", "repair_action"})
    if not has_core_fields:
        return False

    symptom = (record.get("symptom") or "").strip()
    cause = (record.get("possible_cause") or "").strip()
    measure = (record.get("measure") or record.get("repair_action") or "").strip()
    ...
    for block in split_pipe_blocks(lines):
        ...
        for row in block[1:]:
            ...
            if is_separator_record(record):
                continue
            if is_symptom_scene(scene_id) and is_probably_noise_record_for_symptom(record, fields):
                continue
...
def merge_field_patterns(adaptation: Dict[str, Any]) -> Dict[str, List[str]]:
    merged: Dict[str, List[str]] = {}

    def add_patterns(raw: Any) -> None:
        ...
        for key, value in raw.items():
            ...
            if isinstance(value, list):
                ...
                patterns = [value.strip()] if value.strip() else []
            else:
                patterns = []
            if not patterns:
                continue
            ...
            for pattern in patterns:
                if pattern not in merged[str(key)]:
                    merged[str(key)].append(pattern)

    add_patterns(adaptation.get("field_patterns", {}))
    online = adaptation.get("online_refinement", {})
    if isinstance(online, dict):
        add_patterns(online.get("field_patterns", {}))
    return merged
...
def parse_lines(lines: List[str], fields: List[str], alias_map: Dict[str, List[str]], adaptation: Dict[str, Any], scene_id: str) -> List[Dict[str, str]]:
    ...
    records: List[Dict[str, str]] = []
    field_patterns = merge_field_patterns(adaptation)
    for raw_line in lines:
        ...
        if any(value for value in parsed.values()):
            if is_separator_record(parsed):
                continue
            if not (is_symptom_scene(scene_id) and is_probably_noise_record_for_symptom(parsed, fields)):
...
def extract_from_text(text: str, config: Dict[str, Any], source_name: str) -> List[Dict[str, str]]:
    ...
    alias_map = resolve_alias_map(config, fields)
    ...
    if is_symptom_scene(scene_id):
        records = [r for r in records if not is_probably_noise_record_for_symptom(r, fields)]
