...
- **脚本运行**: Python 3.10+
- **模板**: 本地 JSON 场景模板（13 个场景，9 主 + 5 子）
...
- `current`：当前激活步骤
- `done`：已完成，可进入下一步
- `stale`：上游数据变更后自动降级，下游数据被清除

**路由守卫**（`src/main.ts`）：每次路由切换前检查目标步骤是否满足进入条件（前置步骤必须为 `done`），不满足则重定向到当前激活步骤。

**状态降级**：上游（Step1-3）数据变更时，自动将下游步骤状态降级为 `stale` 并清除关联数据。
...
Python 解析策略优先级：
1. **表格**：按 `|` 分割，适配维修手册中的 ASCII 表格
2. **键值对**：使用 `field_patterns` 正则匹配字段
3. **编号步骤**：按数字编号提取步骤
4. **全文截断**：无法解析时保留前 500 字

症状场景（`symptom_table`）有额外的表头门控和噪声过滤逻辑。
...
- `commands/runner.rs`：Python 执行前校验
  - `run_id`：防止路径遍历攻击，校验生成的目录名
  - `python_bin`：白名单校验（仅允许 `python`、`python3`）
  - 输入/输出路径：校验路径安全，禁止 `..` 路径 traversal
- 错误处理：`Result<T, String>` 返回，内部用 `anyhow`
- LLM 调用失败时有本地降级逻辑（非致命）
- Python 文件通过 `include_str!` 编译进二进制
- `enforce_rule_package()` 后处理规则包：
  - 强制所有字段必须来自场景模板的 `output_schema`
  - 裁剪模板中不存在的别名
  - 合并重复的验证规则
  - 确保 `mapping_rules` 和 `validation_rules` 与模板字段对应

### 场景模板

- 模板定义在 `src/config/scene-templates/*.json`（13 个文件）
- 每个模板包含：`scene_id`、`context_keywords`、`header_alias`、`output_schema`、`mapping_rules`（部分）、`validation_rules`、`examples`（部分）
- 成熟度差异大：仅 `symptom_table` 和 `diagnostic_flow` 有 examples，仅 `symptom_table` 有 `mapping_rules`
- 场景注册表在 `src/config/sceneRegistry.ts`（`PRIMARY_SCENES` 数组），需同步更新
